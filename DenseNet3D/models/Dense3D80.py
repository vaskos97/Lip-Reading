...
class _Transition_80(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        super(_Transition_80, self).__init__()
        self.add_module('norm', nn.BatchNorm3d(num_input_features))
        self.add_module('relu', nn.ReLU(inplace=True))
        self.add_module('conv', nn.Conv3d(num_input_features, num_output_features,
                                          kernel_size=1, stride=1, bias=False))
...
class Dense3D(torch.nn.Module):
    def __init__(self, options, growth_rate=32, num_init_features=64, bn_size=4, drop_rate=0):
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i == len(block_config) - 2:
                trans = _Transition_80(num_input_features=num_features, num_output_features=num_features // 2)
                self.features.add_module('transition%d' % (i + 1), trans)
                num_features = num_features // 2
